...
void card_state::set0xff() {
	set_max_property_val(code);
	set_max_property_val(code2);
	set_max_property_val(type);
	set_max_property_val(level);
	set_max_property_val(rank);
	set_max_property_val(link);
	set_max_property_val(link_marker);
	set_max_property_val(lscale);
	set_max_property_val(rscale);
	set_max_property_val(attribute);
	set_max_property_val(race);
	set_max_property_val(attack);
	set_max_property_val(defense);
	...
	set_max_property_val(reason);
	set_max_property_val(reason_player);
}
...
uint32_t card::get_code() {
	...
	if (has_valid_property_val(temp.code))
		return temp.code;
	effect_set effects;
...
uint32_t card::get_type(card* scard, uint64_t sumtype, uint8_t playerid) {
	...
	if (has_valid_property_val(temp.type))
		return temp.type;
	effect_set effects;
	...
	if (changed)
		type = alttype;
	set_max_property_val(temp.type);
	return type;
}
...
int32_t card::get_base_attack() {
	...
	set_max_property_val(temp.base_attack);
	return batk;
}
...
int32_t card::get_base_defense() {
	...
	set_max_property_val(temp.base_defense);
	return bdef;
}
...
uint32_t card::get_attribute(card* scard, uint64_t sumtype, uint8_t playerid) {
	...
	if (changed)
		attribute = altattribute;
	set_max_property_val(temp.attribute);
	return attribute;
}
// see get_level()
uint64_t card::get_race(card* scard, uint64_t sumtype, uint8_t playerid) {
	...
	if (changed)
		race = altrace;
	set_max_property_val(temp.race);
	return race;
}
uint32_t card::get_lscale() {
	if(!current.is_location(LOCATION_PZONE))
		return data.lscale;
	if (has_valid_property_val(temp.lscale))
		return temp.lscale;
	effect_set effects;
...
uint32_t card::get_rscale() {
	if(!current.is_location(LOCATION_PZONE))
		return data.rscale;
	if (has_valid_property_val(temp.rscale))
		return temp.rscale;
	effect_set effects;
	...
	rscale += up + upc;
	set_max_property_val(temp.rscale);
	return rscale;
}
uint32_t card::get_link_marker() {
	...
	if(!(get_type() & TYPE_LINK))
		return 0;
	if (has_valid_property_val(temp.link_marker))
	...
	set_max_property_val(temp.link_marker);
...
void card::xyz_add(card* mat) {
	...
	mat->pre_overlay_target = nullptr;
...
void card::xyz_remove(card* mat) {
	...
	mat->pre_overlay_target = std::exchange(mat->overlay_target, nullptr);
...
int32_t card::add_effect(effect* peffect) {
	...
	else if(peffect->code == EVENT_DETACH_MATERIAL)
		pduel->game_field->core.global_flag |= GLOBALFLAG_DETACH_EVENT;
...
int32_t card::is_fusion_summonable_card(uint32_t summon_type) {
	...
	for(const auto& peffect : eset) {
		pduel->lua->add_param<LuaParam::EFFECT>(pduel->game_field->core.reason_effect);
		pduel->lua->add_param<LuaParam::INT>(pduel->game_field->core.reason_player);
		pduel->lua->add_param<LuaParam::INT>(summon_type);
		pduel->lua->add_param<LuaParam::INT>(0);
		pduel->lua->add_param<LuaParam::INT>(0);
...
int32_t card::is_spsummonable(effect* peffect) {
	...
	pduel->lua->add_param<LuaParam::EFFECT>(peffect);
	pduel->lua->add_param<LuaParam::CARD>(this);
	pduel->lua->add_param<LuaParam::GROUP>(pduel->game_field->core.must_use_mats);
	pduel->lua->add_param<LuaParam::GROUP>(pduel->game_field->core.only_use_mats);
	uint32_t result = FALSE;
	uint32_t param_count = 4;
	if(pduel->game_field->core.forced_summon_minc) {
		pduel->lua->add_param<LuaParam::INT>(pduel->game_field->core.forced_summon_minc);
		pduel->lua->add_param<LuaParam::INT>(pduel->game_field->core.forced_summon_maxc);
		param_count += 2;
	}
...
int32_t card::is_summonable(effect* peffect, uint8_t min_tribute, uint32_t zone, uint32_t releasable, effect* exeffect) {
	...
	pduel->lua->add_param<LuaParam::EFFECT>(exeffect);
...
int32_t card::is_destructable_by_effect(effect* peffect, uint8_t playerid) {
	...
	for(const auto& peff : eset) {
		pduel->lua->add_param<LuaParam::EFFECT>(peffect);
		pduel->lua->add_param<LuaParam::INT>(REASON_EFFECT);
		pduel->lua->add_param<LuaParam::INT>(playerid);
		if(peff->check_value_condition(3)) {
			return FALSE;
		}
...
int32_t card::is_can_be_synchro_material(card* scard, uint8_t playerid, card* /*tuner*/) {
	...
	filter_effect(EFFECT_CANNOT_BE_MATERIAL, &eset);
	for(const auto& peffect : eset) {
		pduel->lua->add_param<LuaParam::INT>(SUMMON_TYPE_SYNCHRO);
		pduel->lua->add_param<LuaParam::INT>(playerid);
...
int32_t card::is_can_be_ritual_material(card* scard, uint8_t playerid) {
	...
	filter_effect(EFFECT_CANNOT_BE_MATERIAL, &eset);
	for(const auto& peffect : eset) {
		pduel->lua->add_param<LuaParam::INT>(SUMMON_TYPE_RITUAL);
		pduel->lua->add_param<LuaParam::INT>(playerid);
		if(peffect->get_value(scard, 2))
			return FALSE;
	}
...
int32_t card::is_can_be_xyz_material(card* scard, uint8_t playerid, uint32_t reason) {
	...
	if(reason & REASON_MATERIAL) {
		...
		filter_effect(EFFECT_CANNOT_BE_MATERIAL, &eset);
		for(const auto& peffect : eset) {
			pduel->lua->add_param<LuaParam::INT>(SUMMON_TYPE_XYZ);
			pduel->lua->add_param<LuaParam::INT>(playerid);
			if(peffect->get_value(scard, 2))
				return FALSE;
		}
...
int32_t card::is_can_be_link_material(card* scard, uint8_t playerid) {
	...
	filter_effect(EFFECT_CANNOT_BE_MATERIAL, &eset);
	for(const auto& peffect : eset) {
		pduel->lua->add_param<LuaParam::INT>(SUMMON_TYPE_LINK);
		pduel->lua->add_param<LuaParam::INT>(playerid);
		if(peffect->get_value(scard, 2))
			return FALSE;
	}
