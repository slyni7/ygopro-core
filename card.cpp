		...
		if (query_flag & QUERY_SQUARE)	{
			...
			if (get_square_mana_count()) {
				for (int i = 1; i <= get_square_mana_count(); ++i) {
					q_cache.nth_square_mana[i - 1] = *p++ = get_nth_square_mana(i);
				}
			}
		}
	}
...
int32 card::add_counter(uint8 playerid, uint16 countertype, uint16 count, uint8 singly) {
	...
	pduel->game_field->raise_single_event(this, 0, EVENT_ADD_COUNTER + countertype, pduel->game_field->core.reason_effect->get_active_effect(), REASON_EFFECT, playerid, playerid, pcount);
	pduel->game_field->process_single_event();
	return TRUE;
}
...
int32 card::filter_summon_procedure(uint8 playerid, effect_set* peset, uint8 ignore_count, uint8 min_tribute, uint32 zone) {
	...
	if(!ignore_count
		...
		for(int32 i = 0; i < eset.size(); ++i) {
			...
			int32 extra_count = retval.size() > 3 ? retval[3] : 1;
			if(new_min < min)
				new_min = min;
			new_zone &= zone;
			if((pduel->game_field->core.extra_summon[playerid] < extra_count)
				&& pduel->game_field->check_tribute(this, new_min, max, 0, current.controler, new_zone, releasable))
				return TRUE;
		}
	} else
...
int32 card::check_summon_procedure(effect* proc, uint8 playerid, uint8 ignore_count, uint8 min_tribute, uint32 zone) {
	...
	if(!ignore_count
		...
		for(int32 i = 0; i < eset.size(); ++i) {
			...
			new_zone &= zone;
			if((pduel->game_field->core.extra_summon[playerid] < extra_count)
				&& is_summonable(proc, new_min_tribute, new_zone, releasable))
				return TRUE;
		}
	} else
...
int32 card::filter_set_procedure(uint8 playerid, effect_set* peset, uint8 ignore_count, uint8 min_tribute, uint32 zone) {
	...
	if(!ignore_count
		...
		for(int32 i = 0; i < eset.size(); ++i) {
			...
			new_zone &= zone;
			if((pduel->game_field->core.extra_summon[playerid] < extra_count)
				&& pduel->game_field->check_tribute(this, new_min, max, 0, current.controler, new_zone, releasable, POS_FACEDOWN_DEFENSE))
				return TRUE;
		}
	} else
...
int32 card::is_summonable_card() {
	...
	return (eset.size() > 0) || (!(data.type & (TYPE_RITUAL | TYPE_SPSUMMON
		...
		| TYPE_TOKEN | TYPE_TRAPMONSTER)) && !is_affected_by_effect(EFFECT_UNSUMMONABLE_CARD));
}
