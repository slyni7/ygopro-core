...
bool field::process(Processors::IdleCommand& arg) {
	...
	case 0: {
		...
		if((infos.phase == PHASE_MAIN1 && is_player_affected_by_effect(idle_player, EFFECT_SKIP_M1))
			...
			if(core.to_bp && core.to_ep) {
				...
				emplace_process<Processors::SelectOption>(idle_player);
				arg.step = 11;
			} else if(core.to_bp) {
		...
		for(auto& pcard : player[idle_player].list_hand)
			if(pcard->is_can_be_summoned(idle_player, FALSE, nullptr, 0))
				core.summonable_cards.push_back(pcard);
		for(auto& pcard : player[idle_player].list_mzone) {
			if(pcard && pcard->is_can_be_summoned(idle_player, FALSE, nullptr, 0))
				core.summonable_cards.push_back(pcard);
		}
		...
		emplace_process<Processors::SelectIdleCmd>(idle_player);
		return FALSE;
	}
	case 1: {
		...
		if(ctype == 5) {
			...
			core.select_chains.clear();
			emplace_process<Processors::AddChain>();
			emplace_process<Processors::QuickEffect>(false, 1 - idle_player);
			infos.priorities[0] = 0;
			infos.priorities[1] = 0;
			core.select_chains.clear();
			if (idlecmdchk)
	...
	case 7: {
		...
		if(target->is_position(POS_FACEUP_ATTACK)) {
			...
			change_position(target, nullptr, idle_player, POS_FACEUP_DEFENSE, FALSE);
			adjust_all();
			emplace_process<Processors::PointEvent>(false, false, false);
		} else if(target->is_position(POS_FACEUP_DEFENSE)) {
			core.phase_action = true;
			change_position(target, nullptr, idle_player, POS_FACEUP_ATTACK, FALSE);
			adjust_all();
			emplace_process<Processors::PointEvent>(false, false, false);
			...
			emplace_process<Processors::SelectPosition>(idle_player, target->data.code, positions);
			arg.step = 12;
			return FALSE;
		} else
	...
	case 13: {
		...
		if(returns.at<int32_t>(0) == POS_FACEUP_ATTACK)
			...
			core.phase_action = true;
			change_position(target, nullptr, idle_player, POS_FACEDOWN_DEFENSE, FALSE);
			adjust_all();
			emplace_process<Processors::PointEvent>(false, false, false);
		}
		target->set_status(STATUS_FORM_CHANGED, TRUE);
...
bool field::process(Processors::BattleCommand& arg) {
	...
	case 0: {
		...
		if(auto peffect = is_player_affected_by_effect(btl_player, EFFECT_SKIP_BP); peffect != nullptr || core.force_turn_end) {
			arg.step = 41;
			arg.repeat_battle_phase = static_cast<bool>(is_player_affected_by_effect(btl_player, EFFECT_BP_TWICE));
		...
		emplace_process<Processors::SelectBattleCmd>(btl_player);
		return FALSE;
	}
	case 1: {
		...
		if(arg.forced_attack_done || (ctype != 0 && ctype != 1)) {
			...
			if(core.tpchain.size() > 1) {
				emplace_process<Processors::SortChain>(btl_player);
				arg.step = 13;
			}
			else if(core.tpchain.size() == 1){
				core.sub_solving_event.push_back(nil_event);
				emplace_process<Processors::ExecuteOperation>(core.tpchain.front().triggering_effect, btl_player);
				adjust_all();
			}
			return FALSE;
		} else {
	...
	case 4: {
		...
		if(core.attacker->direct_attackable) {
			...
			if(is_player_affected_by_effect(infos.turn_player, EFFECT_PATRICIAN_OF_DARKNESS))
				emplace_process<Processors::SelectEffectYesNo>(1 - btl_player, 31, core.attacker);
			else
				emplace_process<Processors::SelectYesNo>(btl_player, 31);
			return FALSE;
		}
		...
		if((atype == 3 && differentMustAttackMonsterEffects != 1) || is_player_affected_by_effect(btl_player, EFFECT_PATRICIAN_OF_DARKNESS)) {
			...
			message->write<uint64_t>(549);
			emplace_process<Processors::SelectCard>(btl_player, core.attack_cancelable, 1, 1);
		}
		arg.step = 5;
		return FALSE;
	}
	...
	case 8: {
		...
		message->write(core.attacker->get_info_location());
		if(core.attack_target) {
			raise_single_event(core.attack_target, nullptr, EVENT_BE_BATTLE_TARGET, nullptr, 0, 0, 1 - btl_player, 0);
			raise_event(core.attack_target, EVENT_BE_BATTLE_TARGET, nullptr, 0, 0, 1 - btl_player, 0);
		...
		if(!arg.is_replaying_attack) {
			raise_single_event(core.attacker, nullptr, EVENT_ATTACK_ANNOUNCE, nullptr, 0, 0, btl_player, 0);
			raise_event(core.attacker, EVENT_ATTACK_ANNOUNCE, nullptr, 0, 0, btl_player, 0);
		}
		core.attacker->attack_controler = core.attacker->current.controler;
		core.pre_field[0] = core.attacker->fieldid_r;
	...
	case 11: {
		...
		if(is_flag(DUEL_STORE_ATTACK_REPLAYS) && !core.chain_attack) {
			...
			emplace_process<Processors::SelectYesNo>(btl_player, 30);
		else {
	...
	case 14: {
		for(auto clit = core.tpchain.begin(); clit != core.tpchain.end(); ++clit) {
			core.sub_solving_event.push_back(nil_event);
			emplace_process<Processors::ExecuteOperation>(clit->triggering_effect, btl_player);
			adjust_all();
		}
		core.tpchain.clear();
		arg.step = 2;
	...
	case 26: {
		...
		if(!damchange) {
			if(core.battle_damage[btl_player]) {
				raise_single_event(core.attacker, nullptr, EVENT_PRE_BATTLE_DAMAGE, nullptr, 0, reason_card->current.controler, btl_player, core.battle_damage[btl_player]);
				if(core.attack_target)
					raise_single_event(core.attack_target, nullptr, EVENT_PRE_BATTLE_DAMAGE, nullptr, 0, reason_card->current.controler, btl_player, core.battle_damage[btl_player]);
				raise_event(reason_card, EVENT_PRE_BATTLE_DAMAGE, nullptr, 0, reason_card->current.controler, btl_player, core.battle_damage[btl_player]);
			}
			if(core.battle_damage[1 - btl_player]) {
				raise_single_event(core.attacker, nullptr, EVENT_PRE_BATTLE_DAMAGE, nullptr, 0, reason_card->current.controler, 1 - btl_player, core.battle_damage[1 - btl_player]);
				if(core.attack_target)
					raise_single_event(core.attack_target, nullptr, EVENT_PRE_BATTLE_DAMAGE, nullptr, 0, reason_card->current.controler, 1 - btl_player, core.battle_damage[1 - btl_player]);
				raise_event(reason_card, EVENT_PRE_BATTLE_DAMAGE, nullptr, 0, reason_card->current.controler, 1 - btl_player, core.battle_damage[1 - btl_player]);
			}
		}
		process_single_event();
		process_instant_event();
	...
	case 42: {
		...
		if(arg.repeat_battle_phase && arg.second_battle_phase_is_optional) {
			emplace_process<Processors::SelectYesNo>(btl_player, 32);
			return FALSE;
		}
		returns.set<int32_t>(0, arg.phase_to_change_to);
